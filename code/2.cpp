...
int main()
{
    ...
    int *nuull_pointer = nullptr; // 初始化指针为0(NULL,nullptr 三者一样的)
    void *test_pointer = &a1;
    //*test_pointer = 3;
...
*/
    ...
    // r3=3
    /*
    原理:由于r3是对const int的引用而dval是double,因此编译器为了能够使r3绑定一个整数
    就把代码变成了
    const int temp=dval;
    const int &r3=temp;
    这个temp就是临时量对象
    */
    ...
    // 指针与const
    // 与引用类似，指向常量的指针(const int *)也不能通过解引用来改变值，无论这个指针指向的是常量还是变量，但是指针的指向可以更改(常量指针)
    // 而指针常量(int *const)定义时必须初始化,且之后不能改变指针的指向(值),不过可以通过解引用改变指向对象的值
    // 顶层const表示指针本身是常量,而底层const表示指针所指向的对象是常量
    // 更一般的概念，顶层可以表示这个数据本身是常量。而底层则表示引用，指针等复合类型的对象是常量
    int i = 0;
    ...
    p2 = p3; // 正确,虽然p2没有包括顶层const,但是对于赋值、拷贝而言,顶层const作为右值对于它本身没有什么影响
    ...
    *p4 = 3;
    ...
    auto a5 = ci; // a5是int(这书好像有些小问题,书上给的是const int 可能是因为我的是c++17)
